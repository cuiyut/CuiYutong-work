...
let tb:HTMLTableSectionElement = document.getElementsByTagName('tbody')[0]
/**
 * 渲染
 */
function load(arr:Array<leiXing>) {
    tb.innerHTML = ''
    arr.forEach(item => {
    ...
    let total:HTMLSpanElement = document.getElementsByTagName('span')[0]
 ...
 * 编码排序
 */
let idd:boolean = true
function idxu(div1:HTMLDivElement) {
    if(idd) {
...
function prixu(div2:HTMLDivElement) {
    if(ispri) {
        json.sort(function(a,b) {
 ...
 * 搜索
 */
function sou(inp:HTMLInputElement) {
    let val = inp.value
    var arr1 = json.filter(item => {
